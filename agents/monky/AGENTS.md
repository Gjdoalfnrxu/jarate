...
- Host: marzuki-hydrogen (100.95.144.25), Fedora 42 x64 — deployed here under user `monky`
- Inference GPU: marzuki-hydrogen (100.95.144.25) — **2x RTX PRO 5000 Blackwell 48GB** (GPU 0 + GPU 1, nvidia-smi confirmed 2026-09-14), local to the agent host (changed from neon 2026-09-05). marzuki-neon (100.91.94.56) remains the AMD GPU box: RX 9070 XT 16GB (gfx1201), Ryzen 7 9700X 8C/16T, 32GB RAM, Fedora 43 KDE Plasma, kernel 6.19, ROCm 7.7. Access: ssh andryo@100.91.94.56 (agent key authorized since 2026-08-25); LAN 192.168.1.88; WireGuard wg-mrzk-NZ-21 10.2.0.2. llama.cpp in ~/.local/bin (llama-server, llama-cli); server may be down. Check rocm-smi before GPU work on neon.
...
- Andryo Marzuki - <@108801968763305984>
  - aliases: andy, fungus
...
- Cain Cresswell Miley - <@215356028869541889>
...
- Joshua Stachyshyn - <@108702440215298048>
  - aliases: josh, j, jash
...
- Peter Antontios - <@108705148775211008>
  - aliases: pete, fehras
...
- marzuki-hydrogen (100.95.144.25)
...
## Roster
- monky (you) — channel 1545018100590846033 (pi + jarate bridge), host: marzuki-hydrogen, user `monky`
- frank (Franky) — channel 1546723442282205184 (pi + jarate bridge; senior). Migrated 2026-09-09: now on marzuki-hydrogen under user `frank` (same setup as monky: pi.service user unit, ~/.pi/agent, jarate bridge). Old home was vm-frank on helium (decommissioned). To run commands as frank: `sudo -u frank XDG_RUNTIME_DIR=/run/user/1002 systemctl --user ...` (monky has no passwordless sudo yet).
- jimmy — channel 1548547729272344717 (pi + jarate bridge), host: marzuki-hydrogen, user `jimmy` (uid 1004), bot 1548546461950550026. Onboarded 2026-09-13. Earthworm. Uses Andryo's PAT for now (~/.config/marzukia-pat); switchboard key sbk_jimmy_56659a2ca404b4a6 (262K ctx, c=2, P1). To run commands as jimmy: `sudo -u jimmy XDG_RUNTIME_DIR=/run/user/1004 systemctl --user ...`.

## Messaging a peer
- **agent-say is ONLY for bot-to-bot chat.** It posts to another AGENT's channel. Never use it to reach a human. If a human (Cain, Andryo, Pete, Josh) is in your channel, reply in your channel — the bridge auto-forwards. If you need to tag someone in a reply, use `<@userId>` (e.g. `<@215356028869541889>` for Cain).
...
- Incident 2026-09-20: monky used agent-say to Frank's channel to send Cain an infographic link — Franky received a message about Cain's design work. The fix: reply to Cain in your own channel with `<@215356028869541889>`, not agent-say to Frank.
