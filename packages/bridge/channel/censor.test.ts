...
describe("pattern classes", () => {
  ...
  test("sshpass -p <arg>", () => {
    const out = censor("sshpass -p hunter2 ssh andryo@100.91.94.56 ls", {
      file: R,
    });
    expect(out).toBe(
      "sshpass -p [REDACTED:sshpass] ssh andryo@100.91.94.56 ls",
    );
  });
...
describe("registry", () => {
  test("exact literal from secrets.txt, longest-first", () => {
    reg(["phishasu", "phishasu-extended-sudo"]);
    const out = censor(
      "run sudo -S with phishasu-extended-sudo, or just phishasu",
      { file: registryFile },
    );
    expect(out).toBe(
  ...
  test("missing file = patterns only, never an error", () => {
    expect(() =>
      censor("phishasu and ghp_aBc123D456eF78901234567890123456", {
        file: R,
      }),
    ).not.toThrow();
    const out = censor("phishasu and ghp_aBc123D456eF78901234567890123456", {
      file: R,
    });
    expect(out).toContain("phishasu"); // no registry → literal survives
...
describe("behavior", () => {
  ...
  test("registry match logs WARN with fingerprint, not the raw value", () => {
    const lines: string[] = [];
    reg(["phishasupassword"]);
    censor("echo phishasupassword", {
      file: registryFile,
      log: (l) => lines.push(l),
    });
    ...
    expect(lines[0]).not.toContain("phishasupassword"); // no raw value
  });

  test("short registry secret fingerprint is length only (no leak)", () => {
    const lines: string[] = [];
    reg(["phishasu"]); // 8 chars: first4+last4 would be the whole secret
    censor("echo phishasu", { file: registryFile, log: (l) => lines.push(l) });
    expect(lines).toHaveLength(1);
    expect(lines[0]).toContain("8ch");
    expect(lines[0]).not.toContain("phishasu");
  });
